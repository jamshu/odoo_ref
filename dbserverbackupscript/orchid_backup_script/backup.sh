...
for db in $databases ; do
        ssh orchid@188.166.234.220 "pg_dump --cluster 9.5/main --format=c - $db > "/tmp/"$db"_"$filename.dump"
        scp orchid@188.166.234.220:"/tmp/"$db"_"$filename.dump $filename_path"/"$db"_"$filename.dump
        ssh orchid@188.166.234.220 "rm -rf /tmp/"$db"_"$filename.dump
done
